...
(defgeneric window-border-width (window)
  (:method (window)
    (declare (ignore window))
    1))
(defgeneric window-right-border-width (window)
  (:method (window)
    (declare (ignore window))
    1))
(defgeneric window-bottom-border-width (window)
  (:method (window)
    (declare (ignore window))
    1))
(defgeneric window-title-height (window)
  (:method (window)
    (declare (ignore window))
    30))
(defgeneric window-preferred-screen-bounds (window)
  (:method (window)
    (declare (ignore window))
    (values #|tlbr|# 0 0 1024 768)))
(defgeneric view-preferred-size (view)
  (:method (view)
    (declare (ignore view))
    (make-point 512 342)))
(defgeneric view-font-line-height (view)
  (:method (view)
    (declare (ignore view))
    16))

(defvar *deferred-drawing* nil)
(defmacro with-deferred-drawing (view &body body)
  (let ((vview (gensym)))
    `(let ((,vview ,view))
       (multiple-value-prog1 (let ((*deferred-drawing* t)) ,@body)
         (view-draw-contents ,vview)))))

(defgeneric allow-view-draw-contents (view)
  (:method (view)
    (declare (ignore view))
    (values)))
