...
NOTE: should be called insinde (with-mutex (queue-mutex *event-queue*) …)"
  (let ((entry (queue-head *event-queue*)))
    (when entry
      (if (eq entry (queue-tail *event-queue*))
    ...
    (if (eq event (car events))
        ...
        (loop
          :until (or (null (cdr events))
                     (eq event (cadr events)))
          :do (pop events)
          :finally (return (when (cdr events)
                             (when (eq (cdr events) (queue-tail *event-queue*))
...
(defun test/post-event/dequeue-event ()
  (let ((e1 (make-event :what mouse-down :when (tick-count) :where #@(100 100)))
    ...
    (let ((*event-queue* (make-queue)))
      ...
      (post-event e2)
      (post-event e3)
      (let ((e (dequeue-event))) (assert (eq e1 e) (e e1)))
      (let ((e (dequeue-event))) (assert (eq e2 e) (e e2)))
      (let ((e (dequeue-event))) (assert (eq e3 e) (e e3)))
...
(defun test/%find-event ()
  (let ((e1 (make-event :what mouse-down :when (tick-count) :where #@(100 100)))
    ...
    (let ((*event-queue* (make-queue)))
      ...
      (post-event (make-event))
      (with-mutex (queue-mutex *event-queue*)
        (let ((e (%find-event mouse-down-mask))) (assert (eq e e1) (e e1)))
        (let ((e (%find-event key-down-mask)))   (assert (eq e e2) (e e2)))
        (let ((e (%find-event mouse-up-mask)))   (assert (eq e e3) (e e3)))
        (let ((e (%find-event activate-mask)))   (assert (null e)  (e)))
        (let ((e (%find-event (+ mouse-down-mask
                                 mouse-up-mask)))) (assert (eq e e1) (e e1)))))
    :success))
