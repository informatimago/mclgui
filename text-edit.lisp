...
(defun %te-coordinates-at-point (pt te)
  (let* ((h     (point-h pt))
         (v     (point-v pt))
         (lino  (max 0 (truncate (- v (rect-top (te-%dest-rect te)))
                                 (te-line-height te))))
...
(defun te-set-default-bindings (te)
  ...
  (te-bind +ns-up-arrow-function-code+    'te-previous-line-command        te)
  (te-bind +ns-down-arrow-function-code+  'te-next-line-command            te)
  (te-bind +ns-left-arrow-function-code+  'te-backward-char-command        te)
  (te-bind +ns-right-arrow-function-code+ 'te-forward-char-command         te)
  (te-bind +ns-page-up-function-code+     'te-scroll-down-command          te)
  (te-bind +ns-page-down-function-code+   'te-scroll-up-command            te)
  (te-bind +ns-delete-char-function-code+ 'te-delete-forward-char-command  te)
  (te-bind +ns-home-function-code+        'te-beginning-of-buffer-command  te)
  (te-bind +ns-end-function-code+         'te-end-of-buffer-command        te))
